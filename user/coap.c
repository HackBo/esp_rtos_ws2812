...
coap_parseHeader(coap_header_t *hdr, const uint8_t *buf, size_t buflen) {
  if (buflen < 4) return COAP_ERR_HEADER_TOO_SHORT;
  hdr->ver = (buf[0] & 0xC0) >> 6;
  if (hdr->ver != 1) return COAP_ERR_VERSION_NOT_1;
  hdr->t = (buf[0] & 0x30) >> 4;
  hdr->tkl = buf[0] & 0x0F;
  hdr->code = buf[1];
  hdr->id[0] = buf[2];
...
// advances p
int ICACHE_FLASH_ATTR coap_parseOption(coap_option_t *option,
...
int ICACHE_FLASH_ATTR
coap_parseOptionsAndPayload(coap_option_t *options, uint8_t *numOptions,
  ...
  int rc;
  if (p > end) return COAP_ERR_OPTION_OVERRUNS_PACKET;  // out of bounds
  ...
  while ((optionIndex < *numOptions) && (p < end) && (*p != 0xFF)) {
    if (0 !=
        (rc = coap_parseOption(&options[optionIndex], &delta, &p, end - p)))
      return rc;
    optionIndex++;
  }
...
int ICACHE_FLASH_ATTR
coap_parse(coap_packet_t *pkt, const uint8_t *buf, size_t buflen) {
  int rc;

  // coap_dump(buf, buflen, false);

  if (0 != (rc = coap_parseHeader(&pkt->hdr, buf, buflen))) return rc;
  //    coap_dumpHeader(&hdr);
  if (0 != (rc = coap_parseToken(&pkt->tok, &pkt->hdr, buf, buflen))) return rc;
  pkt->numopts = MAXOPT;
  if (0 != (rc = coap_parseOptionsAndPayload(pkt->opts, &(pkt->numopts),
                                             &(pkt->payload), &pkt->hdr, buf,
                                             buflen)))
    return rc;
...
// option num
const coap_option_t *ICACHE_FLASH_ATTR
coap_findOptions(const coap_packet_t *pkt, uint8_t num, uint8_t *count) {
  ...
  *count = 0;
  for (i = 0; i < pkt->numopts; i++) {
    if (pkt->opts[i].num == num) {
      if (NULL == first) first = &pkt->opts[i];
      (*count)++;
    } else {
      if (NULL != first) break;
    }
...
int ICACHE_FLASH_ATTR coap_buffer_to_string(char *strbuf, size_t strbuflen,
                                            const coap_buffer_t *buf) {
  if (buf->len + 1 > strbuflen) return COAP_ERR_BUFFER_TOO_SMALL;
...
int ICACHE_FLASH_ATTR
coap_build(uint8_t *buf, size_t *buflen, const coap_packet_t *pkt) {
  size_t opts_len = 0;
  size_t i;
  uint8_t *p;
  ...
  // build header
  if (*buflen < (4U + pkt->hdr.tkl)) return COAP_ERR_BUFFER_TOO_SMALL;
  ...
  if (pkt->hdr.tkl > 0) memcpy(p, pkt->tok.p, pkt->hdr.tkl);
  ...
  for (i = 0; i < pkt->numopts; i++) {
    ...
    if (((size_t)(p - buf)) > *buflen) return COAP_ERR_BUFFER_TOO_SMALL;
...
int ICACHE_FLASH_ATTR
coap_make_response(coap_rw_buffer_t *scratch, coap_packet_t *pkt,
                   const uint8_t *content, size_t content_len, uint8_t msgid_hi,
                   uint8_t msgid_lo, const coap_buffer_t *tok,
...
int ICACHE_FLASH_ATTR coap_handle_req(coap_rw_buffer_t *scratch,
  ...
  while (NULL != ep->handler) {
    if (ep->method != inpkt->hdr.code) goto next;
    if (NULL != (opt = coap_findOptions(inpkt, COAP_OPTION_URI_PATH, &count))) {
      if (count != ep->path->count) goto next;
      for (i = 0; i < count; i++) {
        if (opt[i].buf.len != strlen(ep->path->elems[i])) goto next;
        if (0 != memcmp(ep->path->elems[i], opt[i].buf.p, opt[i].buf.len))
          goto next;
      }
      // match!
      return ep->handler(scratch, inpkt, outpkt, inpkt->hdr.id[0],
                         inpkt->hdr.id[1]);
    }
  next:
    ep++;
  ...
  coap_make_response(scratch, outpkt, NULL, 0, inpkt->hdr.id[0],
                     inpkt->hdr.id[1], &inpkt->tok, COAP_RSPCODE_NOT_FOUND,
