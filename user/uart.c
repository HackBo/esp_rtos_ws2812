...
enum { UART_EVENT_RX_CHAR, UART_EVENT_MAX };
...
LOCAL void uart_rx_intr_handler_ssc(void *p) {
  /* uart0 and uart1 intr combine togther, when interrupt occur, see reg
   * 0x3ff20020, bit2, bit0 represents
...
void ICACHE_FLASH_ATTR UART_SetBaudrate(UART_Port uart_no, uint32 baud_rate) {
  uart_div_modify(uart_no, UART_CLK_FREQ / baud_rate);
}

// only when USART_HardwareFlowControl_RTS is set , will the rx_thresh value be
// set.
void ICACHE_FLASH_ATTR UART_SetFlowCtrl(UART_Port uart_no,
...
void ICACHE_FLASH_ATTR UART_WaitTxFifoEmpty(
    UART_Port uart_no)  // do not use if tx flow control enabled
{
...
void ICACHE_FLASH_ATTR UART_SetIntrEna(UART_Port uart_no, uint32 ena_mask) {
...
UART_ParamConfig(UART_Port uart_no, UART_ConfigTypeDef *pUARTConfig) {
  ...
  WRITE_PERI_REG(UART_CONF0(uart_no),
                 ((pUARTConfig->parity == USART_Parity_None)
                      ? 0x0
                      : (UART_PARITY_EN | pUARTConfig->parity)) |
                     (pUARTConfig->stop_bits << UART_STOP_BIT_NUM_S) |
                     (pUARTConfig->data_bits << UART_BIT_NUM_S) |
                     ((pUARTConfig->flow_ctrl & USART_HardwareFlowControl_CTS)
                          ? UART_TX_FLOW_EN
                          : 0x0) |
  ...
  while (uart_intr_status != 0x0) {
    if (UART_FRM_ERR_INT_ST == (uart_intr_status & UART_FRM_ERR_INT_ST)) {
      ...
      while (buf_idx < fifo_len) {
        ...
        ret = xQueueSendToBackFromISR(xQueueUart, &RcvChar,
...
char ICACHE_FLASH_ATTR uart_getchar_ms(int timeout) {
  portBASE_TYPE ticks;
  unsigned char ch;
  if (timeout < 0) {
    ticks = portMAX_DELAY;
...
void ICACHE_FLASH_ATTR uart_init_new(void) {
  ...
  // xTaskCreate(uart_task, (uint8 const *)"uTask", 512, NULL, tskIDLE_PRIORITY
  // + 2, &xUartTaskHandle);
